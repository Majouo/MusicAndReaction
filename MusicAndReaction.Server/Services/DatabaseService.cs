...
public class DatabaseService(AppDbContext context) : IDatabaseService
{
    ...
    public async Task<List<double>> GetDataForMusic(int trackid) => await context.UserReactions.Where(x => x.TrackId == trackid).Select(x=>x.ReactionTime).ToListAsync();
    public async Task<List<double>> GetOrderByForMusic(int trackid) => await context.UserReactions.Where(x => x.TrackId == trackid).OrderBy(x => x.ReactionTime).Select(x => x.ReactionTime).ToListAsync();
